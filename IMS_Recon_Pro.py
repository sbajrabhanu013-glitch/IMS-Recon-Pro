...
def login_page():
    ...
    st.markdown(
        ...
        """ + logo_html + """
                <div class='inalsa-login-welcome'>Welcome</div>
                <div class='inalsa-login-sub'>GST IMS Reconciliation Workspace<br>Secure compliance access panel</div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        """
                <div class='inalsa-login-copy'>© @BAJRABHANU • INALSA Home Appliances • IMS Recon Workspace</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def v10_quality_dashboard():
    """
    Upload Data Quality Dashboard — native Streamlit only.
    No HTML card strings are used here, so raw HTML cannot appear on screen.
    ...
    def render_quality(title, df, icon):
        ...
        r1c1.metric("Data Quality", f"{score}%", quality_status(score))
        r1c2.metric("Records", f"{data['records']:,}")
        r1c3.metric("Taxable Value", f"₹{data['taxable']:,.0f}")
        r1c4.metric("Invoice Value", f"₹{data['invoice_value']:,.0f}")

        r2c1, r2c2, r2c3, r2c4 = st.columns(4)
        r2c1.metric("IGST", f"₹{data['igst']:,.0f}")
        r2c2.metric("CGST", f"₹{data['cgst']:,.0f}")
        r2c3.metric("SGST", f"₹{data['sgst']:,.0f}")
        r2c4.metric("CESS", f"₹{data['cess']:,.0f}")

        r3c1, _, _, _ = st.columns(4)
        r3c1.metric("Total Tax", f"₹{data['total_tax']:,.0f}")
